...
def extractLogMessageFromGitCommit(commit):
    ...
    # TODO FIXME: title is first line of commit, not 1st paragraph.
    foundTitle = False
    ...
    def run(self, args):
        ...
        for line in lines:
            if self.rollbackLocalBranches or (line.startswith("p4/") and line != "p4/HEAD\n"):
                ...
                if len(p4Cmd("changes -m 1 " + ' '.join(['%s...@%s' % (p, maxChange) for p in depotPaths]))) == 0:
...
class P4Submit(Command, P4UserMap):
    ...
    def __init__(self):
        ...
        self.options = [
            ...
            optparse.make_option("--shelve", dest="shelve", action="store_true",
                                 help="Shelve instead of submit. Shelved files are reverted, restoring the workspace to the state before the shelve"),
            optparse.make_option("--update-shelve", dest="update_shelve", action="append", type="int",
                                 metavar="CHANGELIST",
                                 help="update an existing shelved changelist, implies --shelve, repeat in-order for multiple shelved changelists")
        ]
    ...
    def p4UserForCommit(self, id):
        ...
        self.getUserMapFromPerforceServer()
        gitEmail = read_pipe(git_build_cmd('log', '--max-count', '1', '--format', '%ae', id)).strip()
        return (self.emails.get(gitEmail, None), gitEmail)
    ...
    def canChangeChangelists(self):
        ...
        for r in results:
            if r.get('perm', None) in ['admin', 'super']:
                return True
        return False
...
"""
        ...
        for key, value in change_entry.iteritems():
            if key.startswith('File'):
                if 'depot-paths' in settings:
                    if not [p for p in settings['depot-paths'] if p4PathStartsWith(value, p)]:
                        continue
                else:
...
class P4Sync(Command, P4UserMap):
    ...
    def extractFilesFromCommit(self, commit):
        ...
        fnum = 0
        while True:
            path = commit.get("depotFile%s" % fnum, None)
            if path is None:
                break

            if (
                any(p4PathStartsWith(path, p) for p in self.depotPaths) and
                not any(p4PathStartsWith(path, p) for p in self.cloneExclude)
            ):
                file = {}
                file["path"] = path
    ...
    def extractJobsFromCommit(self, commit):
        ...
        jnum = 0
        while True:
            job = commit.get("job%s" % jnum, None)
            if job is None:
                break
            jobs.append(job)
            jnum = jnum + 1
    ...
    def splitFilesIntoBranches(self, commit):
        ...
        fnum = 0
        while True:
            path = commit.get("depotFile%s" % fnum)
            if path is None:
                break
    ...
    def commit(self, details, files, branch, parent=""):
        ...
        files = [f for f in files if self.inClientSpec(f['path']) and self.hasBranchPrefix(f['path'])]

        if not files and not gitConfigBool('git-p4.keepEmptyCommits'):
            print 'Ignoring revision %s as it would produce an empty commit.' % details['change']
            return
        ...
        if change in self.labels:
            ...
            files = p4CmdList(["files"] + ["%s...@%s" % (p, change) for p in self.branchPrefixes])
    ...
    def getLabels(self):
        ...
        for output in l:
            ...
            for file in p4CmdList(["files"] + ["%s...@%s" % (p, label) for p in self.depotPaths]):
    ...
    def updateOptionDict(self, d):
        ...
        if self.keepRepoPath:
            option_strings.append('keepRepoPath')
        d['options'] = ' '.join(sorted(option_strings))

    def readOptions(self, d):
        option_strings = d.get('options', '').split()
        self.keepRepoPath = ('keepRepoPath' in option_strings)
    ...
    def run(self, args):
        ...
        # TODO FIXME: should always look at previous commits,
        ...
        if len(args) == 0 and self.depotPaths:
            ...
            if self.depotPaths and self.depotPaths != args:
                print "previous import used depot path %s and now %s was specified. This doesn't work!" % (
                    ' '.join(self.depotPaths),
                    ' '.join(args),
                )
                sys.exit(1)
        ...
        if self.detectBranches:
            # TODO FIXME - what's a P4 projectName ?
...
class P4Clone(P4Sync):
    ...
    def defaultDestination(self, args):
        # TODO FIXME: use common prefix of args?
        depotPath = args[0]
